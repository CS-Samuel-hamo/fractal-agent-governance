...
```text
<USER_HOME>\.roo\agent-governance-kit
```
...
- CLI runtime: `agent bootstrap`, `agent run`, `/goal`, `/loop`, task
  classification, metrics, and execution control
...
For CLI-first operation, run `agent bootstrap` once per project. Use
`agent-setup` when global Roo/Zoo entrypoints also need repair. Use
