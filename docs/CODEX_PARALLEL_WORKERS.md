...
Codex CLI can be used as a parallel worker process, but the CLI runtime owns
the scheduler. Zoo Code may visualize or trigger this path, but it is optional.
...
```powershell
--worker-codex-home-root "<CODEX_HOME_WORKERS>"
```
