...
- A unified seven-section interaction summary:
  `Result`, `Where you are`, `What changed`, `How to check`, `Next`,
  `If this is not what you wanted`, and `Details`.
- A beginner-friendly Welcome state for empty projects or first runs.
- `agent do "<task>"` for independent one-off tasks that should not replace the
  current project goal.
- Clearer `agent continue` recommendations: continue is only the primary next
  step when there is a real queued action to run.
- Details sections for Project Map, landing proof, Project Rules, and Logic
  Check so the product still exposes its operator model without overwhelming
  the first screen.

### Validation

- Re-ran unified prompt entry, background job mode, project logic/rules check,
  Cockpit data/render, UX simplification, product surface hardening, product
  alpha, starter pack, and whitespace checks.

## 1.0.7 Unified Prompt Execution Entry Patch

This patch makes `agent "<prompt>"` the primary product entry: the operator
understands the prompt, executes one safe step when possible, and reports the
result in product language.

### Added

- Prompt intent routing for single-step edits, seed prompt execution, project
  goals, and unsafe requests.
- Safer job switching so preview-only or no-delivery jobs do not trap the user
  behind `Existing job found`.
- Compatibility support for `--preview` and `--apply` without making them the
  main user mental model.

### Safety

- Full project prompts are converted into Project Map-backed progress, not
  blindly executed end to end.
- Dangerous requests such as reading `.env`, deleting files, pushing, merging,
  deploying, or database migration still require attention.

## 1.0.6 Timeout-aware Worker Failover & Docs Apply Patch

This patch fixes the case where a low-risk documentation update could fail
through Codex timeout, Windows sandbox issues, or path handling and then appear
as a dry run instead of a real result.

### Added

- Safe bounded docs writer for README and `docs/` `.md` / `.txt` files.
- Timeout-aware actual execution results that do not report dry-run as apply
  success.
- Optional remote OpenAI worker adapter plumbing, disabled by default and using
  only environment variables for API keys.
- Worker diagnostics for Codex timeout, sandbox, Unicode path, and unavailable
  worker cases.

### Safety

- The bounded docs writer does not read secrets, modify source code, run shell
  commands, push, merge, deploy, or overwrite unrelated files.
- API keys are never written to `.zoo-agent/` artifacts.
