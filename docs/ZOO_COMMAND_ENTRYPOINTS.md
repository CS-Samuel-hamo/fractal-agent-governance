# CLI and Zoo Entrypoints

This template exposes the AI-native execution loop through a CLI-first runtime.
Roo/Zoo command files may call the same scripts, but they are optional UI
entrypoints rather than the only control surface.

## Default Command

Use CLI `agent run <input>` for normal engineering work.
...
When operating through Zoo/Roo, `.roo/commands/agent-run.md` should delegate to
the same CLI runtime.

The CLI routes to:

```text
<workspace>/scripts/route_task.py
or <USER_HOME>\.roo\agent-governance-kit\scripts\route_task.py
```

Use the workspace router when present. Use the installed global governance kit
router when an old business project has no local bridge scripts.

## Bootstrap Command

Use `agent bootstrap` for CLI-first runtime initialization. Use
`.roo/commands/agent-setup.md` only when you also need to refresh global
Roo/Zoo command files or legacy project bootstrap artifacts.
...
5. Run `agent run <input>` for the actual task.
...
```powershell
python <USER_HOME>\.roo\agent-governance-kit\scripts\sync_zoo_entrypoints.py `
...
- injects the CLI runtime override into local `.roo/commands/agent-run.md`
...
`agent run <input>` as the default path.
