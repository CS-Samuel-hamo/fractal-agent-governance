...
This bridge now runs behind the CLI-first Agent Runtime. The runtime should
optimize for cheap isolated attempts, not heavy pre-approval.
...
`scripts/run_ai_native_task.py` remains an execution backend. It is no longer
the primary user-facing runtime entrypoint.
