...
from public_alpha_packager import write_manifest  # noqa: E402
from public_docs_leakage_scanner import scan_project, scan_texts  # noqa: E402
...
def test_demo_fixture_has_no_leaks() -> None:
    ...
    texts = {path.relative_to(ROOT).as_posix(): path.read_text(encoding='utf-8-sig', errors='replace') for path in demo_root.rglob('*') if path.is_file()}
