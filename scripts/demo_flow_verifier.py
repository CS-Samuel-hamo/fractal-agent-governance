...
from public_release_packager import public_release_dir
...
def copy_demo(project: Path) -> Path:
    ...
    shutil.copytree(
        ...
        target,
        dirs_exist_ok=True,
        ignore=shutil.ignore_patterns('.zoo-agent', '.git', '__pycache__', '.pytest_cache'),
    )
...
def run_agent(project: Path, workspace: Path, args: list[str]) -> dict[str, Any]:
    ...
    proc = subprocess.run(
        [sys.executable, str(project / 'scripts' / 'agent.py'), *args, '--workspace', str(workspace)],
        cwd=workspace,
        env=env,
        text=True,
        encoding='utf-8',
...
def verify(project: Path) -> dict[str, Any]:
    ...
    mismatches = []
    demo_doc = (
        (project / 'DEMO.md').read_text(encoding='utf-8-sig', errors='replace')
        if (project / 'DEMO.md').exists()
        else ''
    )
