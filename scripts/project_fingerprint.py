...
def build_fingerprint(project: Path) -> dict[str, Any]:
    ...
    risk_profile = sorted(
        {str(item.get('severity') or 'unknown') for item in project_map.get('risks') or [] if isinstance(item, dict)}
    )
...
def main() -> int:
    ...
    output = (
        ...
        if args.output
        else project / '.zoo-agent' / 'learning' / 'cross_project' / 'project_fingerprint.json'
    )
