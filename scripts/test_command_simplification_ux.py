...
def test_docs_prioritize_two_command_flow() -> None:
    ...
    combined = '\n'.join((ROOT / doc).read_text(encoding='utf-8-sig') for doc in ['README.md', 'QUICKSTART.md', 'CLI_REFERENCE.md', 'FAQ.md', 'NEW_PROJECT_GUIDE.md'])
...
def test_command_ux_linter_passes() -> None:
    ...
    readiness = json.loads((ROOT / '.zoo-agent' / 'command_ux' / 'readiness_for_first_user_test.json').read_text(encoding='utf-8-sig'))
