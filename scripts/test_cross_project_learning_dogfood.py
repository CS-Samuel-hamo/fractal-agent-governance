...
from cross_project_learning_dogfood_runner import run_dogfood  # noqa: E402
...
def assert_no_private_content(project: Path) -> None:
    text = '\n'.join(path.read_text(encoding='utf-8', errors='replace') for path in (project / '.zoo-agent').rglob('*') if path.is_file())
    forbidden = ['SHOULD-NOT-BE-READ', 'FAKE_TOKEN_12345', 'raw backend log that should not be stored', 'sk-test-12345', str(project)]
...
def main() -> int:
    with tempfile.TemporaryDirectory(prefix='agent-learning-dogfood-') as tmp:
        ...
        assert_true(result.get('readiness_value') == 'READY_FOR_098_GITHUB_PR_RELEASE_WORKFLOW', 'dogfood did not reach 0.98 readiness')
        ...
        assert_true(all(item.get('baseline') and item.get('learning_enabled') for item in runs), 'baseline or learning mode missing')
        assert_true(all((item.get('safety') or {}).get('blocked_zone_respected') for item in runs), 'blocked zone was not preserved')
        assert_true(all((item.get('safety') or {}).get('checkpoint_required') for item in runs), 'checkpoint requirement was not preserved')
        ...
        assert_true(any(item.get('release_sequence_improved') for item in comparisons), 'no positive release readiness lift')
        assert_true(any(item.get('worker_preference_improved') for item in comparisons), 'no positive worker routing lift')
        assert_true(any(item.get('failure_warning_added') for item in comparisons), 'no failure warning lift')
        assert_true(not any(item.get('negative_lift_detected') for item in comparisons), 'unexpected negative learning detected')
        ...
        assert_true(readiness.get('readiness') == 'READY_FOR_098_GITHUB_PR_RELEASE_WORKFLOW', 'readiness conclusion wrong')
        ...
        assert_true('Worker preference' in cockpit_text or 'Worker' in cockpit_text, 'cockpit missing worker preference')
        assert_true('learning_insights.json' not in cockpit_text and 'raw learning artifacts' not in cockpit_text.lower(), 'cockpit leaked raw learning artifacts')
        ...
        negative = {
            ...
            'summary': {'total_projects': 1, 'projects_with_positive_lift': 0, 'projects_with_negative_lift': 1, 'neutral_projects': 0},
        }
        write_json(negative_path, negative)
        negative_lift = evaluate_lift(project, comparison_path=negative_path, output_path=dogfood / 'negative_lift_report.json')
        ...
        privacy_lift = evaluate_lift(project, comparison_path=comparison_path, output_path=dogfood / 'privacy_lift_report.json')
        ...
        assert_true('learning --dogfood' not in help_text and 'learning --build' not in help_text, 'ordinary help exposed hidden learning commands')

        cli_workspace = project / 'cli_workspace'
        cli_workspace.mkdir()
        cli_result = run([PYTHON, str(ROOT / 'scripts' / 'agent.py'), 'learning', '--dogfood', '--workspace', str(cli_workspace)])
        assert_true('READY_FOR_098_GITHUB_PR_RELEASE_WORKFLOW' in cli_result.stdout, 'agent learning --dogfood did not report readiness')
