...
def write_fast_optimistic_report(
    repo: Path, run_id: str, task_id: str, *, changed_files: list[str], final_message: str = 'Done.'
) -> None:
    write(
...
def main() -> int:
    ...
    assert (
        'AGENTS.md' in no_diff_delta['unchanged_existing_diff'] or 'AGENTS.md' in no_diff_delta['governance_artifacts']
    )
    ...
    write_fast_cli_report(
        docs_delivered, 'run-docs-delivered', 'task-docs-delivered', 'fix typo in README', tests_status='not_applicable'
    )
    capture_baseline(
        docs_delivered, 'run-docs-delivered', 'task-docs-delivered', env, text='fix typo in README', task_type='docs'
    )
    write(docs_delivered / 'README.md', '# Demo\n\nTypo fixed.\n')
    write_fast_optimistic_report(
        docs_delivered, 'run-docs-delivered', 'task-docs-delivered', changed_files=['README.md']
    )
    ...
    run(
        [
            sys.executable,
            str(ROOT / 'scripts' / 'runtime_review.py'),
            '--workspace',
            str(docs_delivered),
            '--run-id',
            'run-docs-delivered',
    ...
    capture_baseline(
        ...
        'task-runtime-only',
        env,
        text='fix a local bug in src/example.py',
        task_type='coding',
        allowed_files=['src/example.py'],
    )
    write(runtime_only / '.zoo-agent' / 'runs' / 'run-runtime-only' / 'evidence.txt', 'runtime only\n')
    write_fast_optimistic_report(
        runtime_only, 'run-runtime-only', 'task-runtime-only', changed_files=['.zoo-agent/tmp/evidence.txt']
    )
    ...
    write_fast_optimistic_report(
        governance_only, 'run-governance-only', 'task-governance-only', changed_files=['AGENTS.md']
    )
    ...
    assert proc.returncode == 1
    governance_outcome = read_json(
        governance_only / '.zoo-agent' / 'runs' / 'run-governance-only' / 'delivery-outcome.json'
    )
    ...
    write_fast_optimistic_report(
        bootstrap_delivery, 'run-bootstrap-delivery', 'task-bootstrap-delivery', changed_files=['AGENTS.md']
    )
    ...
    bootstrap_outcome = read_json(
        bootstrap_delivery / '.zoo-agent' / 'runs' / 'run-bootstrap-delivery' / 'delivery-outcome.json'
    )
    ...
    init_repo(no_op, env)
    write_fast_cli_report(
        no_op, 'run-no-op', 'task-no-op', 'fix specified typo in README', tests_status='not_applicable'
    )
    ...
    write_fast_cli_report(
        ...
        'run-empty-no-op',
        'task-empty-no-op',
        'fix specified typo in README',
        tests_status='not_applicable',
    )
    capture_baseline(
        empty_no_op, 'run-empty-no-op', 'task-empty-no-op', env, text='fix specified typo in README', task_type='docs'
    )
    write_fast_optimistic_report(
        empty_no_op, 'run-empty-no-op', 'task-empty-no-op', changed_files=[], final_message='nothing to change'
    )
    ...
    run_may_fail(
        [
            sys.executable,
            str(ROOT / 'scripts' / 'check_project_readiness.py'),
            '--workspace',
            str(dirty),
            '--output',
            str(dirty / '.zoo-agent' / 'project-readiness.json'),
        ],
        ROOT,
        env,
    )
    ...
    run_may_fail(
        [
            ...
            str(unborn),
            '--output',
            str(unborn / '.zoo-agent' / 'project-readiness.json'),
        ],
        ROOT,
        env,
    )
    ...
    run_may_fail(
        [
            ...
            str(nested),
            '--output',
            str(nested / '.zoo-agent' / 'project-readiness.json'),
        ],
        ROOT,
        env,
    )
    ...
    assert par['route'] == 'parallel' or par.get('parallel_denial_reason'), (
        'parallel path must execute or explain denial'
    )
    ...
    run(
        ['git', 'worktree', 'add', '-b', f'zoo/real-usage-alpha/task-rollback-{os.getpid()}', str(worktree), 'HEAD'],
        existing,
        env,
    )
    evidence_dir = existing / '.zoo-agent' / 'runs' / 'run-rollback' / 'optimistic-runs'
    evidence_dir.mkdir(parents=True, exist_ok=True)
    (evidence_dir / 'task-rollback.json').write_text(
        json.dumps({'task_id': 'task-rollback', 'attempts': [{'worktree': str(worktree)}]}, indent=2), encoding='utf-8'
    )
    run(
        [
            AGENT,
