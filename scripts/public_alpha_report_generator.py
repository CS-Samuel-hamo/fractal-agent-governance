...
def generate_report(project: Path, *, refresh: bool = True) -> dict[str, Any]:
    ...
    readiness = {
        ...
        'recommended_next_steps': ['prepare v1.0 public alpha release notes'] if status == 'READY_FOR_100_PUBLIC_ALPHA_RELEASE' else ['fix failed public alpha audit checks'],
    }
    write_json(out / 'readiness_for_100.json', readiness)
    payload = {'status': status, 'report_path': '.zoo-agent/public_alpha/public_alpha_report.md', 'readiness': readiness}
    return payload
