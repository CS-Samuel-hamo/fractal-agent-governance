...
import importlib.util, subprocess, tempfile, shutil, os, sys, json
...
def main():
    ...
    integrator_rule = (ROOT/'.roo'/'rules-agent-integrator'/'03-codex-worker-merge-policy.md').read_text(encoding='utf-8')
    ...
    (tmp/'tests'/'test_example.py').write_text('from src.example import add\n\ndef test_add():\n    assert add(1,2)==3\n', encoding='utf-8')
    (tmp/'.zoo-agent'/'project-charter.json').write_text(json.dumps({
    ...
    (tmp/'.zoo-agent'/'current-run.json').write_text(json.dumps({
        'run_id': 'run-test',
        'goal_id': 'goal-main',
    }, ensure_ascii=False, indent=2), encoding='utf-8')
    (tmp/'.zoo-agent'/'goals'/'goal-main.json').write_text(json.dumps({
        'goal_id': 'goal-main',
    ...
    run(['python', str(ROOT/'scripts'/'agent.py'), 'bootstrap',
         '--workspace', str(tmp),
         '--goal-id', 'goal-main',
         '--goal', 'Preserve the sample arithmetic API while improving bounded implementation details.',
         '--force'], cwd=tmp, env=env)
    ...
    run(['python', str(ROOT/'scripts'/'agent.py'), 'standards', 'check',
         '--workspace', str(tmp)], cwd=tmp, env=env)
    run(['python', str(ROOT/'scripts'/'agent.py'), 'map', 'check',
         '--workspace', str(tmp)], cwd=tmp, env=env)
    run(['python', str(ROOT/'scripts'/'agent.py'), 'status',
         '--workspace', str(tmp)], cwd=tmp, env=env)
    ...
    run(['python', str(ROOT/'scripts'/'agent.py'), 'bootstrap',
         '--workspace', str(tmp),
         '--goal-id', 'goal-main',
         '--goal', 'Preserve the sample arithmetic API while improving bounded implementation details.'], cwd=tmp, env=env)
    if not (tmp/'AGENTS.md').exists():
    ...
    run(['python', str(ROOT/'scripts'/'agent.py'), 'bootstrap',
         ...
         '--goal-id', 'goal-main',
         '--goal', 'Preserve the sample arithmetic API while improving bounded implementation details.',
         '--refresh-instructions'], cwd=tmp, env=env)
    ...
    run(['python', str(ROOT/'scripts'/'agent.py'), 'standards', 'promote',
         '--workspace', str(tmp)], cwd=tmp, env=env)
    ...
    run(['python', str(ROOT/'scripts'/'agent.py'), 'map', 'refresh',
         '--workspace', str(tmp)], cwd=tmp, env=env)
    if not (tmp/'.zoo-agent'/'project-map.json.new').exists():
        raise SystemExit('Expected map refresh to write project-map proposal')
    run(['python', str(ROOT/'scripts'/'agent.py'), 'map', 'promote',
         '--workspace', str(tmp)], cwd=tmp, env=env)
    if (tmp/'.zoo-agent'/'project-map.json.new').exists():
        raise SystemExit('Expected map promote to consume project-map proposal')
    run(['python', str(ROOT/'scripts'/'get_goal.py'),
         '--workspace', str(tmp),
         '--goal-id', 'goal-main',
         '--require'], cwd=tmp, env=env)
    alignment = run(['python', str(ROOT/'scripts'/'check_goal_alignment.py'),
         '--workspace', str(tmp),
         '--run-id', 'run-cli',
         '--task-id', 'task-align',
         '--goal-id', 'goal-main',
         '--objective', 'Preserve arithmetic API behavior while adding a bounded implementation comment'], cwd=tmp, env=env)
    ...
    classifier = run(['python', str(ROOT/'scripts'/'task_classifier.py'),
         '--workspace', str(tmp),
         '--input-text', 'Fix sample arithmetic API implementation comment',
         '--allowed-file', 'src/**',
         '--allowed-file', 'tests/**'], cwd=tmp, env=env)
    ...
    parallel_classifier = run(['python', str(ROOT/'scripts'/'task_classifier.py'),
         '--workspace', str(tmp),
         '--input-text', 'Update src/example.py\nUpdate tests/test_example.py'], cwd=tmp, env=env)
    ...
    run(['python', str(ROOT/'scripts'/'route_task.py'),
         '--workspace', str(tmp),
         '--run-id', 'run-cli',
         '--task-id', 'task-cli-fast',
         '--goal-id', 'goal-main',
         '--fast',
         '--allowed-file', 'src/**',
         '--allowed-file', 'tests/**',
         '--input-text', 'Preserve sample arithmetic API with a bounded implementation comment',
         '--dry-run'], cwd=tmp, env=env)
    ...
    run(['python', str(ROOT/'scripts'/'agent.py'), 'run',
         '--workspace', str(tmp),
         '--run-id', 'run-agent',
         '--task-id', 'task-agent-fast',
         '--goal-id', 'goal-main',
         '--fast',
         '--allowed-file', 'src/**',
         '--allowed-file', 'tests/**',
         '--dry-run',
         'Preserve', 'sample', 'arithmetic', 'API', 'comment'], cwd=tmp, env=env)
    ...
    run(['python', str(ROOT/'scripts'/'agent.py'), 'reroute',
         '--workspace', str(tmp),
         '--run-id', 'run-agent',
         '--task-id', 'task-agent-fast',
         '--path', 'governed',
         '--dry-run',
         '--no-execute-governed-workers'], cwd=tmp, env=env)
    ...
    for metric_key in ['fast_path_rate', 'parallel_execution_rate', 'governed_path_rate', 'codex_latency', 'doc_overproduction_rate', 'code_delivery_rate']:
    ...
    run(['git', 'worktree', 'add', '-b', 'zoo/rollback-smoke/task-rollback', str(rollback_worktree), 'HEAD'], tmp, env=env)
    rollback_run_dir = tmp/'.zoo-agent'/'runs'/'run-rollback'/'optimistic-runs'
    rollback_run_dir.mkdir(parents=True, exist_ok=True)
    (rollback_run_dir/'task-rollback.json').write_text(json.dumps({
        'run_id': 'run-rollback',
        'task_id': 'task-rollback',
        'attempts': [{'task_id': 'task-rollback', 'worktree': str(rollback_worktree)}],
    }, ensure_ascii=False, indent=2), encoding='utf-8')
    run(['python', str(ROOT/'scripts'/'agent.py'), 'rollback',
         '--workspace', str(tmp),
         '--run-id', 'run-rollback',
    ...
    run(['python', str(ROOT/'scripts'/'generate_codex_task_pack.py'),
         '--run-id','run-test','--task-id','task-001','--objective','Change add to support integers and add a comment',
         '--allowed-file','src/**','--allowed-file','tests/**','--denied-file','.env','--denied-file','pyproject.toml',
         '--acceptance','Scope guard passes','--test-command','python -m pytest'], cwd=tmp, env=env)
    ...
    run(['python', str(ROOT/'scripts'/'generate_codex_task_pack.py'),
         '--run-id','run-test','--task-id','task-fast','--objective','Fast path prompt smoke test',
         '--allowed-file','src/**','--allowed-file','tests/**','--test-command','python -m pytest',
         '--prompt-template','CODEX_TASK_PROMPT_FAST.md','--output', str(fast_out)], cwd=tmp, env=env)
    assert 'Fast path contract' in (fast_out/'CODEX_TASK_PROMPT.md').read_text(encoding='utf-8'), 'missing fast prompt content'
    ...
    run(['python', str(ROOT/'scripts'/'check_codex_scope.py'), 'task-001', '--tasks', str(out/'TASKS.yaml')], cwd=tmp, env=env)

    # Simulate denied change
    (tmp/'pyproject.toml').write_text('[tool]\n', encoding='utf-8')
    p = run(['python', str(ROOT/'scripts'/'check_codex_scope.py'), 'task-001', '--tasks', str(out/'TASKS.yaml')], cwd=tmp, check=False, env=env)
    if p.returncode == 0:
        raise SystemExit('Expected scope guard failure for denied file')

    # Collect result
    run(['python', str(ROOT/'scripts'/'collect_codex_result.py'), '--run-id','run-test','--task-id','task-001','--task-dir', str(out), '--workspace', str(tmp)], cwd=tmp, env=env)
    ...
    run(['python', str(ROOT/'scripts'/'build_task_context.py'),
         '--run-id','run-test','--task-id','task-context-probe','--workspace', str(tmp),
         '--user-request','Review the entire project and clean up bounded issues',
         '--goal-id','goal-main'], cwd=tmp, env=env)
    ...
    run(['python', str(ROOT/'scripts'/'generate_codex_task_pack.py'),
         '--run-id','run-test','--task-id','task-context-pack','--objective','Task context copy smoke test',
         '--allowed-file','src/**','--allowed-file','tests/**',
         '--task-context', str(context_path),
         '--output', str(context_pack)], cwd=tmp, env=env)
    ...
    fast_select = run(['python', str(ROOT/'scripts'/'select_execution_path.py'),
         '--run-id','run-test','--task-id','task-select-fast',
         '--objective','Fix local rendering bug',
         '--allowed-file','src/**','--allowed-file','tests/**',
         '--test-command','python -m pytest'], cwd=tmp, env=env)
    ...
    high_risk = run(['python', str(ROOT/'scripts'/'select_execution_path.py'),
         '--run-id','run-test','--task-id','task-select-planned',
         '--objective','Change authentication token validation',
         '--allowed-file','src/auth/**','--allowed-file','tests/**',
         '--test-command','python -m pytest'], cwd=tmp, env=env)
    if '"recommended_path": "planned_worker"' not in high_risk.stdout:
        raise SystemExit('Expected planned_worker for hard-risk task')
    level2 = run(['python', str(ROOT/'scripts'/'select_execution_path.py'),
         '--run-id','run-test','--task-id','task-select-level2',
    ...
    level3 = run(['python', str(ROOT/'scripts'/'select_execution_path.py'),
         '--run-id','run-test','--task-id','task-select-level3',
         '--objective','Implement a multi-module workstream',
         '--governance-level','3',
         '--allowed-file','src/**','--allowed-file','tests/**'], cwd=tmp, env=env)
    if '"recommended_path": "fractal_governed"' not in level3.stdout:
        raise SystemExit('Expected fractal_governed for Level 3')
    level4 = run(['python', str(ROOT/'scripts'/'select_execution_path.py'),
         '--run-id','run-test','--task-id','task-select-level4',
         '--objective','Change production release flow',
         '--governance-level','4',
         '--allowed-file','src/**','--allowed-file','tests/**'], cwd=tmp, env=env)
    if '"recommended_path": "human_gate"' not in level4.stdout:
        raise SystemExit('Expected human_gate for Level 4')
    run(['python', str(ROOT/'scripts'/'run_ai_native_task.py'),
         '--run-id','run-test','--task-id','task-dispatch-dry','--workspace', str(tmp),
         '--objective','Dispatcher dry run smoke test',
         '--goal-id','goal-main',
         '--allowed-file','src/**','--allowed-file','tests/**',
         '--test-command','python -m pytest','--dry-run'], cwd=tmp, env=env)
    ...
    selection = json.loads((tmp/'.zoo-agent'/'runs'/'run-test'/'executor-selection.json').read_text(encoding='utf-8'))
    ...
    run(['python', str(ROOT/'scripts'/'run_ai_native_task.py'),
         '--run-id','run-test','--task-id','task-dispatch-planned','--workspace', str(tmp),
         '--objective','Change authentication token validation',
         '--goal-id','goal-main',
         '--allowed-file','src/auth/**','--allowed-file','tests/**',
         '--test-command','python -m pytest'], cwd=tmp, env=env)
    planned_pack = tmp/'.zoo-agent'/'runs'/'run-test'/'codex-tasks'/'task-dispatch-planned'/'CODEX_TASK_PROMPT.md'
    ...
    run(['python', str(ROOT/'scripts'/'run_ai_native_task.py'),
         '--run-id','run-test','--task-id','task-dispatch-fractal','--workspace', str(tmp),
         '--objective','Implement a multi-module workstream',
         '--governance-level','3',
         '--goal-id','goal-main',
         '--allowed-file','src/**','--allowed-file','tests/**'], cwd=tmp, env=env)
    ...
    leaf_index = tmp/'.zoo-agent'/'runs'/'run-test'/'fractal-workstreams'/'task-dispatch-fractal'/'leaf-tasks'/'leaf-tasks.json'
    ...
    broad_parallel_check = run(['python', str(ROOT/'scripts'/'check_codex_worker_concurrency.py'),
         '--workspace', str(tmp),
         '--run-id', 'run-test',
         '--leaf-index', str(leaf_index),
         '--max-workers', '2'], cwd=tmp, check=False, env=env)
    ...
    broad_scheduler = run(['python', str(ROOT/'scripts'/'run_codex_parallel_workers.py'),
         '--workspace', str(tmp),
         '--run-id', 'run-test',
         '--leaf-index', str(leaf_index),
         '--max-workers', '2',
         '--dry-run'], cwd=tmp, check=False, env=env)
    ...
    manual_index.write_text(json.dumps({
        'parent_task_id': 'parallel-smoke',
        'status': 'draft',
        'leaf_count': 2,
        'leaves': [
            {'task_id': leaf_a['task_id'], 'json': str(leaf_a_path), 'conflict_keys': leaf_a['conflict_keys'], 'parallelizable': True},
            {'task_id': leaf_b['task_id'], 'json': str(leaf_b_path), 'conflict_keys': leaf_b['conflict_keys'], 'parallelizable': True},
        ],
    }, ensure_ascii=False, indent=2), encoding='utf-8')
    run(['python', str(ROOT/'scripts'/'run_codex_parallel_workers.py'),
         '--workspace', str(tmp),
         '--run-id', 'run-test',
         '--leaf-index', str(manual_index),
         '--max-workers', '2',
         '--worker-dry-run'], cwd=tmp, env=env)
    parallel_report = json.loads((tmp/'.zoo-agent'/'runs'/'run-test'/'parallel-workers'/'parallel-smoke'/'parallel-run.json').read_text(encoding='utf-8'))
    ...
    merge_queue = json.loads((tmp/'.zoo-agent'/'runs'/'run-test'/'merge-queue.json').read_text(encoding='utf-8'))
    ...
    conflict_index.write_text(json.dumps({
        'parent_task_id': 'parallel-smoke-conflict',
        'status': 'draft',
        'leaf_count': 2,
        'leaves': [
            {'task_id': leaf_a['task_id'], 'json': str(leaf_a_path), 'conflict_keys': leaf_a['conflict_keys'], 'parallelizable': True},
            {'task_id': conflict_leaf_b['task_id'], 'json': str(conflict_leaf_b_path), 'conflict_keys': conflict_leaf_b['conflict_keys'], 'parallelizable': True},
        ],
    }, ensure_ascii=False, indent=2), encoding='utf-8')
    conflict_check = run(['python', str(ROOT/'scripts'/'check_codex_worker_concurrency.py'),
         '--workspace', str(tmp),
         '--run-id', 'run-test',
         '--leaf-index', str(conflict_index)], cwd=tmp, check=False, env=env)
    ...
    run(['python', str(ROOT/'scripts'/'manage_resource_locks.py'),
         '--workspace', str(tmp),
         '--run-id', 'closure-run',
         '--owner', 'owner-a',
         '--conflict-key', 'src/example',
         '--acquire'], cwd=tmp, env=env)
    lock_conflict = run(['python', str(ROOT/'scripts'/'manage_resource_locks.py'),
         '--workspace', str(tmp),
         '--run-id', 'closure-run',
         '--owner', 'owner-b',
         '--conflict-key', 'src/example',
         '--acquire'], cwd=tmp, check=False, env=env)
    ...
    (closure_run/'optimistic-runs'/'closure-task.json').write_text(json.dumps({
        ...
        'attempts': [{
            ...
            'test_results': [{'command': 'python -m pytest tests', 'returncode': 0, 'stdout': '', 'stderr': ''}],
            'collected_result': {
    ...
    (closure_run/'merge-queue.json').write_text(json.dumps({
        'schema_version': '1.0',
    ...
    (closure_run/'goal-alignment'/'closure-task.json').write_text(json.dumps({
        'schema_version': '1.0',
    ...
    run(['python', str(ROOT/'scripts'/'check_task_board_consistency.py'),
         '--workspace', str(tmp),
         '--run-id', 'closure-run'], cwd=tmp, env=env)
    task_board_warning = run(['python', str(ROOT/'scripts'/'check_task_board_consistency.py'),
         '--workspace', str(tmp),
         '--run-id', 'closure-run',
         '--expect-task', 'missing-board-task'], cwd=tmp, check=False, env=env)
    ...
    run(['python', str(ROOT/'scripts'/'check_task_board_consistency.py'),
         '--workspace', str(tmp),
         '--run-id', 'closure-run',
         '--expect-task', 'missing-board-task'], cwd=tmp, env=env)
    run(['python', str(ROOT/'scripts'/'update_risk_register.py'),
         '--workspace', str(tmp),
         '--run-id', 'closure-run',
         '--add',
         '--risk-id', 'risk-001',
         '--title', 'Closure smoke open risk',
         '--severity', 'high',
         '--source', 'smoke'], cwd=tmp, env=env)
    risk_gate = run(['python', str(ROOT/'scripts'/'run_quality_gate.py'),
         '--workspace', str(tmp),
         '--run-id', 'closure-run'], cwd=tmp, check=False, env=env)
    ...
    run(['python', str(ROOT/'scripts'/'update_risk_register.py'),
         ...
         '--close',
         '--risk-id', 'risk-001',
         '--resolution', 'Closed by smoke test'], cwd=tmp, env=env)
    run(['python', str(ROOT/'scripts'/'run_quality_gate.py'),
         '--workspace', str(tmp),
         '--run-id', 'closure-run',
         '--authorize-merge-queue'], cwd=tmp, env=env)
    quality_gate = json.loads((closure_run/'quality-gate.json').read_text(encoding='utf-8'))
    if quality_gate.get('gate_status') != 'pass' or not quality_gate.get('readiness_flags', {}).get('merge_queue_processing_authorized'):
        raise SystemExit('Expected quality gate pass with merge queue authorization')
    run(['python', str(ROOT/'scripts'/'process_merge_queue.py'),
         '--workspace', str(tmp),
         '--run-id', 'closure-run',
         '--authorize',
         '--authorization-note', 'Smoke quality gate passed'], cwd=tmp, env=env)
    ...
    run(['python', str(ROOT/'scripts'/'agent.py'), 'review',
         '--workspace', str(tmp),
         '--run-id', 'closure-run'], cwd=tmp, env=env)
    ...
    run(['python', str(ROOT/'scripts'/'summarize_ai_native_run.py'),
         '--run-id','run-test','--workspace', str(tmp)], cwd=tmp, env=env)
    run(['python', str(ROOT/'scripts'/'run_optimistic_worker.py'),
         '--run-id','run-test','--task-id','task-optimistic','--workspace', str(tmp),
         '--objective','Optimistic dry run smoke test',
         '--allowed-file','src/**','--allowed-file','tests/**',
         '--test-command','python -m pytest','--dry-run'], cwd=tmp, env=env)
    run(['python', str(ROOT/'scripts'/'run_optimistic_worker.py'),
         '--run-id','run-test','--task-id','task-planned-dry','--workspace', str(tmp),
         '--objective','Planned isolated dry run smoke test',
         '--execution-path','planned_worker','--full-prompt','--allow-hard-risk',
         '--allowed-file','src/**','--allowed-file','tests/**',
         '--test-command','python -m pytest','--dry-run'], cwd=tmp, env=env)
    print('Smoke test passed.')
    print('Codex CLI available:', bool(shutil.which('codex')))
    if shutil.which('codex'):
        run(['python', str(ROOT/'scripts'/'run_codex_worker.py'), '--task-dir', str(out), '--workspace', str(tmp), '--dry-run'], env=env)

    local_project = Path(tempfile.mkdtemp(prefix='zoo-local-roo-override-'))
    (local_project/'.roo'/'commands').mkdir(parents=True)
    (local_project/'.roo'/'commands'/'agent-run.md').write_text('# Local Agent Run\n\nKeep local instructions.\n', encoding='utf-8')
    run(['python', str(ROOT/'scripts'/'sync_zoo_entrypoints.py'),
         '--project-root', str(local_project),
         '--project-only',
         '--backup-root', str(local_project/'.roo-backups')], cwd=ROOT, env=env)
    ...
    second = run(['python', str(ROOT/'scripts'/'sync_zoo_entrypoints.py'),
         '--project-root', str(local_project),
         '--project-only',
         '--backup-root', str(local_project/'.roo-backups'),
         '--dry-run'], cwd=ROOT, env=env)
    second_payload = json.loads(second.stdout)
    drift = [item for item in second_payload.get('actions', []) if item.get('action') in {'write', 'copy', 'copy_missing'}]
    if drift:
    ...
    (unified_project/'.zoo-agent'/'project-profile.json').write_text(json.dumps({
        'schema_version': '1.0',
    ...
    (unified_project/'.zoo-agent'/'bootstrap'/'proposals'/'zoo-agent'/'project-profile.json').write_text(json.dumps({
        'schema_version': '1.0',
        'languages': ['Python'],
        'source_roots': [],
    }, ensure_ascii=False, indent=2), encoding='utf-8')
    (unified_project/'.zoo-agent'/'project-readiness.json').write_text(json.dumps({
    ...
    (unified_project/'.zoo-agent'/'project-map.md').write_text('# Project Map\n\n- source: src\n- generated: data/generated.csv\n', encoding='utf-8')
    (unified_project/'.zoo-agent'/'project-map.json').write_text(json.dumps({
        'source_roots': ['src', 'data/generated'],
        'api_entrypoints': [],
    }, ensure_ascii=False, indent=2), encoding='utf-8')
    (unified_project/'.zoo-agent'/'runs'/'run-old'/'task-board-consistency.json').write_text(json.dumps({
        'status': 'warnings',
        'warnings': ['stale redirect plan requires review'],
    }, ensure_ascii=False, indent=2), encoding='utf-8')
    (unified_project/'.roo'/'rules'/'10-project-architecture.md').write_text('# Project Architecture\n\nsource roots: unknown\n', encoding='utf-8')
    ...
    (unified_project/'tests'/'test_example.py').write_text('from src.example import ok\n\n\ndef test_ok():\n    assert ok()\n', encoding='utf-8')
    run(['python', str(ROOT/'scripts'/'agent_bootstrap.py'),
         '--project', str(unified_project),
         '--mode', 'auto',
         '--goal', 'Unified bootstrap smoke test',
         '--codex-home', env['CODEX_HOME']], cwd=ROOT, env=env)
    ...
    scan_policy = json.loads((unified_project/'.zoo-agent'/'bootstrap'/'scan-policy.json').read_text(encoding='utf-8'))
    if 'data/**' not in scan_policy.get('exclude_patterns', []):
        raise SystemExit('Expected generated data path exclusion in scan policy')
    resolver = json.loads((unified_project/'.zoo-agent'/'bootstrap'/'source-of-truth-resolver.json').read_text(encoding='utf-8'))
    resolver_surfaces = {item.get('surface') for item in resolver.get('precedence', [])}
    if '.steward/**' not in resolver_surfaces or '.zoo-agent/runs/<run-id>/*.json and .zoo-agent/current-run.json' not in resolver_surfaces:
