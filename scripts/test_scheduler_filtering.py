...
def test_bootstrap_runtime_goal_does_not_block_production_goal() -> None:
    ...
    set_goal(
        path,
        'runtime-maintenance',
        'Operate this project through the CLI-first AI Agent Runtime with bounded Codex execution.',
        100,
        resource='runtime:*',
    )
    set_goal(
        path,
        'production-low',
        'Improve README onboarding wording for real users.',
        10,
        resource='README.md',
        no_activate=True,
    )
    ...
    assert all(
        item['goal_id'] != 'runtime-maintenance'
        for item in schedule['ranking']
        if item['goal_id'] in {row['goal_id'] for row in schedule['eligible_goals']}
    )
