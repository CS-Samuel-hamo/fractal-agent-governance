...
def execution_status_from_inputs(
    *,
    ...
    if status in {'timeout', 'no_output_timeout'} or codex_returncode == 124:
        return 'timeout', 'codex_worker_timeout', 0.1
    if status in {'spawn_failed', 'exception'}:
        return 'failed', 'codex_worker_spawn_or_exception', 0.1
    if codex_returncode not in {0, None}:
        return 'failed', 'codex_worker_nonzero_returncode', 0.2
...
def build_execution_result_model(
    *,
    ...
    return {
        ...
        'codex_returncode': '' if codex_returncode is None else codex_returncode,
...
def main() -> int:
    parser = argparse.ArgumentParser(description='Classify Codex execution evidence into a stable execution result model.')
