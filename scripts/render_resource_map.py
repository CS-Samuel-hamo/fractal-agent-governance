...
def main() -> int:
    ...
    resource_map = load_json(project / '.zoo-agent' / 'project-resource-map.json') or generate_resource_map(
        project, args.input_text
    )
    paths = write_resource_map(project, resource_map)
    print(
        json.dumps(
            {'status': 'ok', 'paths': paths, 'markdown_preview': render_resource_map_md(resource_map)[:200]},
            ensure_ascii=True,
            indent=2,
        )
    )
