...
def main() -> int:
    ...
    backup_root = (
        ...
        if args.backup_root
        else Path(args.global_roo).resolve() / 'backups' / f'one-touch-setup-{timestamp}'
    )
    steps: list[dict] = []
    payload = {
        'schema_version': '1.0',
    ...
    if not args.skip_validation:
        ...
        steps.append(
            {
                'name': 'starter_pack_validation',
                'status': 'ok' if validation['returncode'] == 0 else 'failed',
                **validation,
            }
        )
    ...
    if not args.skip_global_sync:
        ...
        steps.append(
            {
                'name': 'global_and_kit_sync',
                'status': 'ok' if sync_result['returncode'] == 0 else 'failed',
                **sync_result,
            }
        )
