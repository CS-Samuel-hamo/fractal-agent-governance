...
def run(
    cmd: list[str], cwd: Path, *, env: dict[str, str], check: bool = True, stdin: str | None = None
...
def main() -> int:
    try:
        ...
        first_bootstrap = run(
            [sys.executable, str(AGENT), 'bootstrap', '--workspace', str(existing)], existing, env=env
        )
        ...
        if status_path.exists():
            status_path.unlink()
        run(
            [sys.executable, str(AGENT), 'status', '--workspace', str(existing), '--run-id', 'run-fast', '--no-write'],
            existing,
            env=env,
        )
