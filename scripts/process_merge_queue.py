...
def main() -> int:
    ...
    ap.add_argument('--authorize', action='store_true', help='Explicitly authorize merge queue processing after quality gate pass.')
    ap.add_argument('--approve-merge-readiness', action='store_true', help='Record final merge-readiness approval. Does not run git merge.')
    ...
    if quality_gate.get('gate_status') != 'pass':
        blockers.append({'id': 'quality_gate_not_pass', 'message': 'quality-gate.json must pass before queue processing.'})
    ...
    if not q_flags.get('merge_queue_processing_authorized') and not args.authorize:
        blockers.append({'id': 'queue_processing_not_authorized', 'message': 'Run quality gate with --authorize-merge-queue or pass --authorize here.'})
    if args.authorize and not args.authorization_note:
        blockers.append({'id': 'missing_authorization_note', 'message': '--authorization-note is required when --authorize is used.'})

    candidates = queue.get('candidates') if isinstance(queue.get('candidates'), list) else []
    if not candidates:
        candidates = candidates_from_summary(summary)
    if not candidates:
        blockers.append({'id': 'missing_merge_candidates', 'message': 'No merge candidates are available for queue processing.'})
    ...
    if parent_status == 'review_required' and not args.accept_parent_aggregation:
        blockers.append({'id': 'parent_aggregation_review_required', 'message': 'Parent aggregation still requires review.'})

    approved_for_merge = bool(args.approve_merge_readiness)
    queue_status = 'blocked'
    if not blockers:
        queue_status = 'processable_merge_readiness_approved' if approved_for_merge else 'processable_pending_serial_integrator'
