...
def build_project_map(project: Path, kind: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    for root, dirs, files in os.walk(project, topdown=True):
        ...
        for filename in sorted(files):
            ...
            if Path(rel_path).name in {"package.json", "requirements.txt", "pyproject.toml", "poetry.lock", "package-lock.json"}:
...
def promote_project_map(project: Path, kind: str) -> tuple[dict[str, Any], list[str]]:
    ...
    if (project / PROJECT_MAP_JSON).exists() and not (project / MAP_PRE_PROMOTION_BACKUP_JSON).exists():
        ...
        (project / MAP_PRE_PROMOTION_BACKUP_JSON).write_text((project / PROJECT_MAP_JSON).read_text(encoding="utf-8", errors="replace"), encoding="utf-8")
    ...
    if (project / PROJECT_MAP_MD).exists() and not (project / MAP_PRE_PROMOTION_BACKUP_MD).exists():
        (project / MAP_PRE_PROMOTION_BACKUP_MD).write_text((project / PROJECT_MAP_MD).read_text(encoding="utf-8", errors="replace"), encoding="utf-8")
...
def render_map_promotion_review(project: Path, kind: str) -> str:
    ...
    verdict = "APPROVE" if active and not active.get("_json_error") and not contamination and all(flags_ok.values()) else "BLOCKED"
    lines = [
    ...
    if contamination:
        ...
        lines.append("- PASS: active project map contains no required exclusion pattern contamination in owned paths or entrypoint candidates.")
...
def render_architecture_review(project: Path, kind: str) -> str:
    ...
    if contamination:
        ...
        lines.append("- PASS: generated/runtime/cache/secret/provider/dependency exclusion contamination is not present in active project-map owned paths or entrypoints.")
    lines.extend([
        "",
...
def render_runtime_readiness(project: Path, kind: str) -> str:
    arch_text = (project / ARCHITECTURE_REVIEW_MD).read_text(encoding="utf-8", errors="replace") if (project / ARCHITECTURE_REVIEW_MD).exists() else ""
...
def inspect_project(project: Path, kind: str) -> dict[str, Any]:
    scan_policy = read_json(project / SCAN_POLICY)
    exclude_patterns = scan_policy.get("exclude_patterns") if isinstance(scan_policy.get("exclude_patterns"), list) else []
