...
from github_release_draft_generator import generate as generate_github_release_draft
...
from release_tag_preflight import preflight as tag_preflight
...
def final_status(gate: dict[str, Any], fresh: dict[str, Any], demo: dict[str, Any], package: dict[str, Any]) -> str:
    ...
    if (
        gate.get('recommendation') == 'fail'
        or fresh.get('unsafe_behavior_detected')
        or demo.get('unsafe_behavior_detected')
    ):
        return 'NOT_READY'
...
def generate_report(project: Path, *, run_fresh_clone: bool = True, run_demo: bool = True) -> dict[str, Any]:
    with contextlib.redirect_stdout(io.StringIO()):
        ...
        fresh = (
            verify_fresh_clone(project)
            if run_fresh_clone
            else load_json(public_release_dir(project) / 'fresh_clone_verification.json')
        )
        demo = (
            verify_demo_flow(project)
            if run_demo
            else load_json(public_release_dir(project) / 'demo_flow_verification.json')
        )
    ...
    payload = {
        'status': status,
        'report_path': '.zoo-agent/public_release/public_release_report.md',
        'readiness': readiness,
    }
...
def main(argv: list[str] | None = None) -> int:
    ...
    payload = generate_report(
        project_root(args.workspace), run_fresh_clone=not args.skip_fresh_clone, run_demo=not args.skip_demo
    )
