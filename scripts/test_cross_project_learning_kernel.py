...
def main() -> int:
    with tempfile.TemporaryDirectory(prefix='agent-learning-kernel-') as tmp:
        ...
        assert_true(
            bool(load_json(store / 'project_index.json').get('projects')), 'project index missing imported project'
        )
        assert_true(
            all(item.get('evidence') for item in pattern_library.get('patterns') or []), 'pattern without evidence'
        )
        low_count_patterns = [
            item
            for item in pattern_library.get('patterns') or []
            if int(item.get('success_count') or 0) + int(item.get('failure_count') or 0) <= 1
        ]
        assert_true(
            all(float(item.get('confidence') or 0) <= 0.45 for item in low_count_patterns),
            'low evidence pattern was overconfident',
        )
        template_ids = {item.get('template_id') for item in release_templates.get('templates') or []}
        assert_true(
            {'cli_tool_release', 'python_library_release', 'docs_first_release', 'agent_runtime_release'}
            <= template_ids,
            'release templates missing',
        )
        unavailable_prefer = [
            item
            for item in worker_memory.get('worker_performance') or []
            if item.get('recommended_use') == 'prefer' and item.get('worker_role') in {'claude', 'local'}
        ]
        ...
        assert_true(
            {'no_delivery', 'worker_unavailable', 'blocked_zone_attempt', 'map_hallucination_risk'} <= failure_types,
        ...
        store_text = '\n'.join(
            path.read_text(encoding='utf-8', errors='replace') for path in store.rglob('*') if path.is_file()
        )
        ...
        assert_true(
            'raw backend log that should not be stored' not in store_text.lower(),
            'raw backend log content leaked into learning store',
        )
