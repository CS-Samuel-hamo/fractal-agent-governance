...
def _action_by_id(project_map: dict[str, Any]) -> dict[str, dict[str, Any]]:
    return {str(item.get('action_id') or ''): item for item in project_map.get('next_actions') or [] if isinstance(item, dict)}
...
def main() -> int:
    ...
    trace_path = Path(args.trace).resolve() if args.trace else project / '.zoo-agent' / 'dogfood' / 'autopilot_trace.json'
