...
def main():
    ...
    if 'goal consistency' not in reviewer_rule or 'repeatability' not in reviewer_rule:
        raise SystemExit('Expected reviewer rule to require Vibe Coding cross-validation dimensions')
    if 'original user task objective' not in integrator_rule or 'conflict keys' not in integrator_rule:
    ...
    run(['python', str(ROOT/'scripts'/'check_codex_worker_concurrency.py'),
         '--workspace', str(tmp),
         '--run-id', 'run-test',
         '--leaf-index', str(leaf_index),
         '--max-workers', '2'], cwd=tmp, env=env)
    ...
    conflict_leaf_b['conflict_keys'] = ['src/a']
    ...
    if compatibility_payload.get('kit_version') != '0.3.11-governance-closure-codex-parallel':
