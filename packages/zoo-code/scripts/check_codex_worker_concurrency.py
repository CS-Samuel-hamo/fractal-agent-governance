...
def build_contract(repo_root: Path, run_id: str, leaf_index_path: Path, *, max_workers: int, allow_planned: bool = False) -> dict:
    ...
    return {
        ...
        'rules': [
            'No worktree, no parallel codex worker.',
            'No non-overlapping conflict keys, no parallel codex worker.',
            'No independent task pack/output/result path, no parallel codex worker.',
            'No parent aggregation, no merge.',
        ],
    }
