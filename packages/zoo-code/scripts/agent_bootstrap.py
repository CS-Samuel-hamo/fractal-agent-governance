...
def probe_environment(project: Path) -> dict:
    ...
    payload: dict = {
        ...
        "node": {
            ...
            "version_probe": run_probe(["node", "-v"], frontend if frontend.exists() else project) if node_available else None,
            "abi_probe": run_probe(["node", "-p", "process.versions.modules"], frontend if frontend.exists() else project) if node_available else None,
        },
        "npm": {
            "available": npm_available,
            "version_probe": run_probe(["npm", "-v"], frontend if frontend.exists() else project) if npm_available else None,
        },
        "native_dependencies": {},
    }
...
def architecture_issues(project: Path, proposal_result: dict, environment: dict) -> list[dict]:
    ...
    data_dirty = [
        ...
        if path.startswith(("data/", "data_test/", "outputs/", "artifacts/", "reports/"))
    ]
    if data_dirty:
        issues.append({
...
def write_architecture_hardening_artifacts(project: Path, proposal_result: dict) -> dict:
    ...
    status = "blocked" if any(item["id"] in hard_issue_ids for item in issues) else ("needs_review" if issues else "ready")
    scan_policy = {
        "schema_version": "1.0",
    ...
    (bootstrap_dir / "scan-policy.json").write_text(json.dumps(scan_policy, ensure_ascii=False, indent=2), encoding="utf-8")
    (bootstrap_dir / "source-of-truth-resolver.json").write_text(json.dumps(resolver, ensure_ascii=False, indent=2), encoding="utf-8")
    (zoo_dir / "installed-kit-version.json").write_text(json.dumps(version_payload, ensure_ascii=False, indent=2), encoding="utf-8")
    (zoo_dir / "architecture-compatibility-report.json").write_text(json.dumps(compatibility, ensure_ascii=False, indent=2), encoding="utf-8")
...
def write_report(project: Path, payload: dict) -> None:
    ...
    if compatibility:
        lines.extend([
            ...
            f"- report: `.zoo-agent/architecture-compatibility-report.md`",
...
def main() -> int:
    ...
    if not bootstrap_script:
        steps.append({"name": "project_context_bootstrap", "status": "blocked", "message": "bootstrap_project.py not found"})
        ...
        return 2
    if not sync_script:
        steps.append({"name": "entrypoint_bridge_repair", "status": "blocked", "message": "sync_zoo_entrypoints.py not found"})
        payload["status"] = "blocked"
        if not args.dry_run:
            write_report(project, payload)
        print_json(payload)
    ...
    steps.append({"name": "project_context_bootstrap", "status": "ok" if bootstrap_result["returncode"] == 0 else "failed", **bootstrap_result})
    ...
    steps.append({"name": "entrypoint_bridge_repair", "status": "ok" if repair_result["returncode"] == 0 else "failed", **repair_result})
