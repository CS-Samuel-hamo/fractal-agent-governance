...
def render_tasks_md(run_id: str, goal_id: str, branches: list[dict], active: str, parallel: str, next_mode: str, direction: str) -> str:
    ...
    return "\n".join([
        ...
        "",
        "## Delivery Mode",
        "- current_task_type: coding | docs-only | research | planning | review | blocked",
        "- docs_only_allowed: yes | no",
        "- implementation_queue_exists: yes | no",
        "- ready_codex_task_exists: yes | no",
        "- code_delivery_gate: pass | fail | pending | not_applicable",
        "",
        "## Implementation Queue Summary",
        "| Item | Type | Status | Executor | Expected Artifact | Root Goal Link |",
        "|---|---|---|---|---|---|",
        "| _fill via /implement_ | coding | pending | codex | code/test/config diff | current goal |",
        "",
        "## Follow-up Backlog",
        "- Local optimization ideas go here unless they directly close the current root goal.",
        "",
        "## Current Active Path",
