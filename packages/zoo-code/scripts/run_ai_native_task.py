...
from execution_policy import DEFAULT_DENIED_FILES, ExecutionPolicyInput, safe_name, select_execution_path  # noqa: E402
from build_task_context import build_context, write_context  # noqa: E402
...
def build_leaf_skeletons(args) -> list[dict]:
    ...
    for index, pattern in enumerate(prod_patterns, start=1):
        ...
        leaf['powershell_command'] = ' '.join(shell_quote(part) if any(ch.isspace() for ch in part) else part for part in leaf['dispatcher_command'])
        leaves.append(leaf)
    return leaves
...
def write_fractal_workstream(args, repo_root: Path, selection: dict, task_pack: dict) -> dict:
    ...
    payload = {
        ...
        'leaf_execution_contract': {
            ...
            'required_leaf_fields': ['task_id', 'objective', 'allowed_files', 'test_commands', 'execution_graph', 'conflict_keys'],
            'recommended_governance_level': 1,
    ...
    return {'json': str(workstream_path), 'markdown': str(md_path), 'status': payload['status'], 'leaf_skeletons': leaf_artifacts}
...
def main() -> int:
    ...
    task_context = build_context(repo_root, args.run_id, args.task_id, args.objective, args.goal_id, args.allow_durable_state_update)
    ...
    if path == 'optimistic_worker':
        ...
        report = {
            ...
            'task_context': task_context_paths,
            'execution': execution,
            'recommended_next_action': 'inspect optimistic run report and merge candidate' if execution.get('returncode') == 0 else 'inspect failure policy and escalate if needed',
        }
    ...
    if path in {'planned_worker', 'fractal_governed'}:
        if args.execute_planned and path == 'planned_worker':
            ...
            report = {
                'status': 'planned_executed_isolated' if execution.get('returncode') == 0 else 'planned_execution_failed_or_escalated',
                'selection': selection,
                'task_context': task_context_paths,
                'execution': execution,
                'recommended_next_action': 'inspect planned isolated run report and merge candidate' if execution.get('returncode') == 0 else 'inspect failure policy and escalate if needed',
            }
            return finish(dispatcher_report_path, report, args, repo_root, execution.get('returncode', 10))
        ...
        if path == 'fractal_governed' and pack.get('returncode') == 0:
            ...
            report['recommended_next_action'] = 'review generated leaf skeletons, refine broad leaves, then run each leaf through run_ai_native_task.py'
        return finish(dispatcher_report_path, report, args, repo_root, 0 if report['status'] in {'planned_pack_ready', 'planned_executed', 'fractal_workstream_ready'} else 10)
