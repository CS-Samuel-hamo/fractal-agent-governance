...
def profile(root: Path, codebase_indexing_status: str = "unknown") -> dict[str, Any]:
    return {"schema_version": "1.3", "generated_by": "generate-project-profile.py", "project_root": str(root), "codebase_indexing_status": codebase_indexing_status, "project_charter_path": ".zoo-agent/project-charter.json", "project_charter_md": "docs/project-charter.md", "project_map_path": ".zoo-agent/project-map.json", "architecture_boundaries_path": ".zoo-agent/architecture-boundaries.json", "package_manager": package_manager(root), "languages": languages(root), "frameworks": ["unknown"], "commands": commands(root), "source_roots": dirs(root, {"src", "app", "lib", "packages", "cmd", "internal", "pkg"}), "test_roots": dirs(root, {"test", "tests", "__tests__", "spec", "e2e"}), "api_entrypoints": entrypoints(root), "patterns": patterns(root), "forbidden_paths": [".env*", "**/*secret*", "**/*token*", "**/*.pem", "**/*.key", ".git/**", "node_modules/**", "dist/**", "build/**"], "generated_files": ["unknown"], "risk_paths": {"security": ["auth/**", "security/**", "**/*auth*"], "data": ["migrations/**", "db/**", "database/**", "schema/**"], "payments": ["payment/**", "billing/**", "**/*stripe*"]}, "notes": ["unknown means no reliable evidence was found", "codebase_indexing_status records Zoo Code semantic indexing availability when known", "project-charter records durable mission and decision boundaries", "project-map records module/file/dependency facts separately from code style policy"]}
