...
AGENT_RUN_BLOCK = """## AI-Native Dispatcher Override
...
- Route Level 3 parent work into leaf skeletons; re-route each leaf through the resolved dispatcher.
- Keep project charter, goal contract, project profile, and project map read-only unless the user explicitly requests a durable update and the command passes `--allow-durable-state-update`.
