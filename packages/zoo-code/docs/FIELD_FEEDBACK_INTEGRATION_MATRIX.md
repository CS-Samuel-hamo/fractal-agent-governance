...
| Codex CLI can be a parallel worker, but Zoo must own scheduling. | `scripts/check_codex_worker_concurrency.py`, `scripts/run_codex_parallel_workers.py`, and `docs/CODEX_PARALLEL_WORKERS.md`. | Smoke validates schedule/resource-lock artifacts and worker dry-run execution. |
